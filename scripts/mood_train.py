...
class Model(pl.LightningModule):
    ...
    def test_step(self, batch,batch_idx):
        ...
        patching = Rearrange("b c (h p1) (w p2) -> b (h w) (p1 p2 c)", p1=self.patch_size, p2=self.patch_size)
        batch = batch[0][0]
        ite = batch.shape[0] // self.max_batch_size

        for i in range(ite):
            result = []
            im = batch[(i * self.max_batch_size):(i+1)* self.max_batch_size].clone()
            im = im[:,None]
            h = w = im.shape[-1] // self.patch_size
            c = im.shape[1] # in channels
            b = im.shape[0]
            if isinstance(self.net, VanillaVAE):
                im = rearrange(im, "b c (h p1) (w p2) -> (b c h w) 1 p1 p2", p1=self.patch_size, p2=self.patch_size)
            ...
            with torch.no_grad():
                ...
                for it in range(5):
                    ...
                    if it == 0 :
                        before_opt_recon = recon.clone()


                    weights, llk_component = store_NLL(im, recon, mu, log_var, z, patching )
                    weights_agg += [weights]
                    llk_comp_agg += [dict([(k, v[0]) for k,v in llk_component.items()])] #ploting only use first image
            ...
            with torch.enable_grad():
                ...
                for it in pbar:
                    ...
                    loss = copy_net.loss_function(*[recon, im, mu, log_var], M_N=0)
                    optimizer.zero_grad()
                    loss['loss'].backward()
                    optimizer.step()
            ...
            with torch.no_grad():
                ...
                for it in range(5):
                    ...
                    weights, llk_component = store_NLL(im, recon, mu, log_var, z, patching)
                    weights_agg += [weights]
                    llk_comp_agg_after += [dict([(k, v[0]) for k,v in llk_component.items()])] #ploting only use first image

                after_opt_recon = recon.clone()
                weights_agg = torch.stack(weights_agg,-1)
                NLL_loss_after = compute_NLL(weights_agg)


            #visualization, pick 1st image
            ...
            #regret = (torch.cat(result, 0) > self.thres)
            patch_q = patching(im).quantile(0.9,dim=-1)
            patch_max = patching(im).max(-1)[0]
            regret[patch_q < 0.01] = 0




            mask = regret.repeat(1,1,self.patch_size**2)
            mask = self.depatching(mask)[:,0].float().detach().cpu().numpy()



            fig, ax = plt.subplots(1,5, figsize=(20,5))
            ax[0].imshow((im[0][0]* 255).cpu().numpy(), cmap='gray') #plot the noised image
            ax[0].set_title('Original')
            ax[1].imshow(mask[0], cmap='Reds')
            ax[1].set_title(f'Regret , max regret {regret[0].max()}')
            ax[2].imshow((before_opt_recon[0][0].detach().cpu() * 255).long().to(torch.uint8).numpy(), cmap='gray') #plot the noised image
            tmp = -llk_comp_agg[0]['log_p_x_z'][:,None].repeat(1,1,self.patch_size**2)
            tmp = self.depatching(tmp)[0,0].float().detach().cpu().numpy()
            ax[2].imshow(tmp, alpha=0.3, cmap='Reds')
            ax[2].set_title('Before Optimization Sample Recon')


            ax[3].imshow((after_opt_recon[0][0]* 255).long().to(torch.uint8).detach().cpu().numpy(), cmap='gray') #plot the noised image
            tmp = -llk_comp_agg_after[0]['log_p_x_z'][:,None].repeat(1,1,self.patch_size**2)
            tmp = self.depatching(tmp)[0,0].float().detach().cpu().numpy()
            #ax[3].imshow(mask[0], alpha=0.5, cmap='Reds')
            ax[3].imshow(tmp, alpha=0.3, cmap='Reds')
            ax[3].set_title('After Optimization Sample Recon')


            ax[4].imshow(patch_max[0].view(8,8).cpu().numpy(), cmap='Reds')
            ax[4].set_title(f'Patch max value.')
            plt.savefig('plots/image.png')
            plt.clf()

            wandb_im = wandb.Image('plots/image.png')
            self.logger.experiment.log({'Likelihood regret plot': wandb_im})
